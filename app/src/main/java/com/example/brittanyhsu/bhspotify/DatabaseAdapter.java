...
public final class DatabaseAdapter {
	...
	public void handleMaxDBRow() {
		...
		if (count > MAX_COUNT) {
			...
			Log.i("GnDatabaseMetadataDel", String.valueOf(result_responce));
			Log.i("GnDatabaseSrchHistDel", String.valueOf(result_history));
		}
	}
...
//			initialValues.put(MUSIC_HISTORY_FINGERPRINT, row.getFingerprintData());
    ...
    static byte[] getDataFromURL(String urlString)
    {
        byte[] imageData = null;

        try {

            imageData = new GnAssetFetch(FingerprintActivity.getGnUser(), urlString).data();
        }
        catch (Exception e)
        {
            return null;
